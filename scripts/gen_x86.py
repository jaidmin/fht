#!/usr/bin/env python3
"""
X86 FHT Code Generator

Generates optimized x86 (AVX/SSE) code for Fast Hadamard Transform.
Based on FFHT gen.py, adapted for the fht library.

Usage:
    python gen_x86.py --output fht_x86_optimized.h [--measurements-dir ./measurements]
"""
...
def get_mean_stddev(measurements_dir):
    with open(os.path.join(measurements_dir, 'output.csv'), 'r') as csvfile:
...
def measure_time(code, log_n, type_name, method_name, measurements_dir,
                 benchmark_include=None, benchmark_lib=None, num_it=3):
    if num_it % 2 == 0:
        raise Exception('even number of runs: %d' % num_it)
    with open(os.path.join(measurements_dir, 'to_run.h'), 'w') as output:
        output.write(code)
    ...
    # Build environment with benchmark paths
    env = os.environ.copy()
    if benchmark_include:
        env['BENCHMARK_INCLUDE'] = benchmark_include
    if benchmark_lib:
        env['BENCHMARK_LIB'] = benchmark_lib

    with open('/dev/null', 'wb') as devnull:
        result = subprocess.call(
            'cd %s && make run_%s' % (measurements_dir, type_name),
            shell=True,
            stdout=devnull,
            env=env)
        if result != 0:
            raise Exception('bad exit code from make')
        run_binary = os.path.join(measurements_dir, 'run_%s' % type_name)
        output_csv = os.path.join(measurements_dir, 'output.csv')
        result = subprocess.call(
            '%s --benchmark_repetitions=%d --benchmark_format=csv > %s'
            % (run_binary, num_it, output_csv),
            shell=True,
            stderr=devnull,
            env=env)
        if result != 0:
            raise Exception('bad exit code from benchmark')
    return get_mean_stddev(measurements_dir)


def generate_header_preamble():
    """Generate the header file preamble with include guards and necessary includes."""
    return '''#ifndef FHT_X86_OPTIMIZED_H
#define FHT_X86_OPTIMIZED_H

/**
 * @file fht_x86_optimized.h
 * @brief Machine-optimized x86 (AVX/SSE) Fast Hadamard Transform implementation.
 *
 * This file is auto-generated by scripts/gen_x86.py
 * DO NOT EDIT MANUALLY - regenerate using the optimization script.
 *
 * The optimal threshold values were determined by benchmarking on the target machine.
 */
...
def generate_header_epilogue():
    """Generate the header file epilogue."""
    return '''
#ifdef __cplusplus
}
#endif

#endif /* FHT_X86_OPTIMIZED_H */
'''


def parse_args():
    parser = argparse.ArgumentParser(
        description='Generate optimized x86 FHT code by benchmarking different strategies.')
    parser.add_argument('--output', '-o', type=str, default='fht_x86_optimized.h',
                        help='Output header file path (default: fht_x86_optimized.h)')
    parser.add_argument('--measurements-dir', '-m', type=str,
                        default=os.path.join(SCRIPT_DIR, 'measurements'),
                        help='Directory containing measurement infrastructure')
    parser.add_argument('--hall-of-fame', type=str, default='hall_of_fame_x86.txt',
                        help='Output file for optimization results summary')
    parser.add_argument('--max-log-n', type=int, default=max_log_n,
                        help='Maximum log2(n) to generate code for (default: 30)')
    parser.add_argument('--benchmark-include', type=str, default=None,
                        help='Path to Google Benchmark include directory')
    parser.add_argument('--benchmark-lib', type=str, default=None,
                        help='Path to Google Benchmark library directory')
...
if __name__ == '__main__':
    ...
    hof_file = args.hall_of_fame
    gen_max_log_n = args.max_log_n
    benchmark_include = args.benchmark_include
    benchmark_lib = args.benchmark_lib

    final_code = generate_header_preamble()
    hall_of_fame = []

    for (type_name,
         composite_step_generator) in [('float', float_avx_composite_step),
                                       ('double', double_avx_composite_step)]:
        for log_n in range(1, gen_max_log_n + 1):
            ...
            time = measure_time(res, log_n, type_name,
                                ...
                                measurements_dir, benchmark_include, benchmark_lib)
        ...
        # Generate the dispatch function for this type
        final_code += 'static inline int fht_%s(%s *buf, int log_n) {\n' % (type_name,
                                                              type_name)
        final_code += '  if (log_n == 0) {\n'
        final_code += '    return 0;\n'
        final_code += '  }\n'
        for i in range(1, gen_max_log_n + 1):
    ...
    final_code += generate_header_epilogue()
